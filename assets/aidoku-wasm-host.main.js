...
var AidokuWasmHostBundle = (() => {
  ...
  // parsers/aidoku/wasm-host/index.ts
  var index_exports = {};
  __export(index_exports, {
    FetchSabState: () => FetchSabState,
    GlobalStore: () => GlobalStore,
    WasmEnv: () => WasmEnv,
    base64ToUint8: () => base64ToUint8,
    createFetchSabPool: () => createFetchSabPool,
    createHost: () => createHost,
    resetWasmSourceRuntime: () => resetWasmSourceRuntime,
    tryCreateSharedArrayBuffer: () => tryCreateSharedArrayBuffer,
    writeFetchErrorSab: () => writeFetchErrorSab,
    writeFetchSuccessSab: () => writeFetchSuccessSab
  });

  // parsers/aidoku/wasm-host/fetch-bridge.ts
  var FETCH_RESPONSE_SAB_SIZE = 8 * 1024 * 1024;
  var FETCH_HEADER_SIZE = 32;
  var FetchSabState = /* @__PURE__ */ ((FetchSabState2) => {
    FetchSabState2[FetchSabState2["Pending"] = 0] = "Pending";
    FetchSabState2[FetchSabState2["Success"] = 1] = "Success";
    FetchSabState2[FetchSabState2["Error"] = 2] = "Error";
    return FetchSabState2;
  })(FetchSabState || {});
  function writeFetchSuccessSab(sab, status, url, body) {
    ...
    Atomics.store(view, 0, 1 /* Success */);
  }
  function writeFetchErrorSab(sab, message) {
    const view = new Int32Array(sab, 0, FETCH_HEADER_SIZE / 4);
    const bytes = new Uint8Array(sab);
    ...
    view[3] = 0;
    Atomics.store(view, 0, 2 /* Error */);
  }
  function tryCreateSharedArrayBuffer(byteLength = 4) {
    try {
  ...
  function createFetchSabPool(count = 8, byteLength = FETCH_RESPONSE_SAB_SIZE) {
    const pool = [];
  ...
  function sendWithSyncXhr(request) {
    const xhr = new XMLHttpRequest();
    xhr.open(request.method, request.url, false);
    xhr.timeout = request.timeoutMs ?? 6e4;
    Object.entries(request.headers).forEach(([key2, value]) => {
  ...
  var SyncFetchBridge = class {
    constructor(postToParent) {
      __publicField(this, "postToParent", postToParent);
    ...
    sendAll(requests) {
      ...
      if (requests.length === 1) {
        const fullRequest = { ...requests[0], id: requests[0].id ?? `fetch-${++this.counter}` };
    ...
    sendAllWithAtomics(requests) {
      const jobs = requests.map((request) => {
        const sab = new SharedArrayBuffer(4);
        const flag = new Int32Array(sab);
        this.pending.set(request.id, {
          resolve: (value) => {
            request.__response = value;
      ...
      for (const job of jobs) {
        const timeoutMs = job.request.timeoutMs ?? 6e4;
        Atomics.wait(job.flag, 0, 0, timeoutMs);
        this.pending.delete(job.request.id);
        const error = job.request.__error;
        if (error) throw error;
        const response = job.request.__response;
        if (!response) {
    ...
    sendWithAtomics(request, sab = new SharedArrayBuffer(4)) {
      const flag = new Int32Array(sab);
      let response = null;
      let error = null;
      this.pending.set(request.id, {
      ...
      Atomics.wait(flag, 0, 0, request.timeoutMs ?? 6e4);
  ...
  function parseXhrHeaders(raw) {
    ...
    raw.trim().split(/[\r\n]+/).filter(Boolean).forEach((line) => {
      ...
      headers[key2] = headers[key2] ? `${headers[key2]}, ${value}` : value;
    });
    return headers;
  }

  // parsers/aidoku/wasm-host/env.ts
  var UserDefaults = class {
    constructor() {
  ...
  var WasmEnv = class {
    constructor(fetchBridge) {
      __publicField(this, "memory", null);
      __publicField(this, "store", new GlobalStore());
      __publicField(this, "defaults", new UserDefaults());
      __publicField(this, "stdout", "");
      __publicField(this, "sourceBaseUrl");
      __publicField(this, "sourceId");
      __publicField(this, "fetchBridge");
      __publicField(this, "homePartialLayout", null);
      __publicField(this, "partialMangaResults", []);
  ...
  function asEncoded(item) {
    if (!item) return void 0;
    if (item.kind === "encoded") return item.value;
    if (item.kind === "string") return new TextEncoder().encode(item.value);
    return void 0;
  }
  function asRequest(item) {
    return item?.kind === "request" ? item.value : void 0;
  }

  // node_modules/.pnpm/@variegated-coffee+serde-postcard-ts@0.1.4/node_modules/@variegated-coffee/serde-postcard-ts/dist/index.js
  var bool = () => ({ kind: "bool" });
  var i32 = () => ({ kind: "i32" });
  var i64 = () => ({ kind: "i64" });
  var f32 = () => ({ kind: "f32" });
  ...
  var unitVariant = (name) => ({
    kind: "unit_variant",
    name
  });
  var newtypeVariant = (name, inner) => ({
    kind: "newtype_variant",
    name,
    inner
  });
  var tupleVariant = (name, ...items) => ({
    kind: "tuple_variant",
    name,
    items
  });
  var structVariant = (name, fields) => ({
    kind: "struct_variant",
    name,
    fields
  });
  var enumType = (name, variants) => ({
    kind: "enum",
    name,
    variants
  });
  ...
  var VarintDecodeError = class extends Error {
    constructor(message) {
      super(message);
  ...
  function zigzagEncodeI32(n) {
    return n << 1 ^ n >> 31;
  }
  function zigzagEncodeBigInt(n) {
    return n << 1n ^ n >> 127n;
  }
  function zigzagDecodeI32(n) {
    return n >>> 1 ^ -(n & 1);
  }
  function zigzagDecodeBigInt(n) {
    return n >> 1n ^ 0n - (n & 1n);
  }
  function tryDecodeVarintU16(data, offset = 0) {
  ...
  function tryDecodeVarintU32(data, offset = 0) {
    if (offset < 0 || offset >= data.length) {
      return err(new VarintDecodeError("Offset out of bounds"));
    }
    let result = 0;
  ...
  function tryDecodeVarintU64(data, offset = 0) {
    if (offset < 0 || offset >= data.length) {
      return err(new VarintDecodeError("Offset out of bounds"));
    }
    let result = 0n;
    ...
    while (bytesRead < MAX_ENCODED_LEN.u64) {
      ...
      bytesRead++;
      const dataBits = BigInt(byte & 127);
      result |= dataBits << shift;
      if ((byte & 128) === 0) {
  ...
  function tryDecodeVarintU128(data, offset = 0) {
    if (offset < 0 || offset >= data.length) {
      return err(new VarintDecodeError("Offset out of bounds"));
    }
    let result = 0n;
  ...
  function tryDecodeVarintI16(data, offset = 0) {
  ...
  function tryDecodeVarintI32(data, offset = 0) {
  ...
  function tryDecodeVarintI64(data, offset = 0) {
  ...
  function tryDecodeVarintI128(data, offset = 0) {
  ...
  var VarintEncodeError = class extends Error {
    constructor(message) {
      super(message);
      this.name = "VarintEncodeError";
    }
  };
  function tryEncodeVarintU16(value) {
    ...
    let n = value;
    while (n > 127) {
      bytes2.push(n & 127 | 128);
      n >>>= 7;
    }
    bytes2.push(n & 127);
    return ok({ bytes: new Uint8Array(bytes2), bytesWritten: bytes2.length });
  }
  function tryEncodeVarintU32(value) {
  ...
  function tryEncodeVarintU64(value) {
    ...
    let n = value;
    while (n > 0x7fn) {
      bytes2.push(Number(n & 0x7fn) | 128);
      n >>= 7n;
    }
    bytes2.push(Number(n & 0x7fn));
    return ok({ bytes: new Uint8Array(bytes2), bytesWritten: bytes2.length });
  }
  function tryEncodeVarintU128(value) {
    const maxU128 = (1n << 128n) - 1n;
  ...
  function tryEncodeVarintI16(value) {
  ...
  function tryEncodeVarintI32(value) {
    ...
    const encoded = zigzagEncodeI32(value);
    return tryEncodeVarintU32(encoded >>> 0);
  }
  function tryEncodeVarintI64(value) {
    const minI64 = -(1n << 63n);
    const maxI64 = (1n << 63n) - 1n;
  ...
  function tryEncodeVarintI128(value) {
    const minI128 = -(1n << 127n);
    const maxI128 = (1n << 127n) - 1n;
  ...
  var BoolDecodeError = class extends Error {
    constructor(message) {
      super(message);
      this.name = "BoolDecodeError";
    }
  };
  function tryDecodeBool(data, offset = 0) {
    if (offset < 0 || offset >= data.length) {
  ...
  function tryEncodeBool(value) {
    const byte = value ? 1 : 0;
    return ok({ bytes: new Uint8Array([byte]), bytesWritten: 1 });
  }
  var NumberDecodeError = class extends Error {
    constructor(message) {
      super(message);
      this.name = "NumberDecodeError";
    }
  };
  function tryDecodeU8(data, offset = 0) {
    if (offset < 0 || offset >= data.length) {
  ...
  function tryDecodeI8(data, offset = 0) {
    if (offset < 0 || offset >= data.length) {
      return err(new NumberDecodeError("Offset out of bounds"));
    }
    const byte = data[offset];
  ...
  function tryDecodeF32(data, offset = 0) {
    if (offset < 0 || offset + 4 > data.length) {
      return err(new NumberDecodeError("Not enough bytes for f32"));
    }
    const bytes2 = data.slice(offset, offset + 4);
    const view = new DataView(bytes2.buffer, bytes2.byteOffset, 4);
    const value = view.getFloat32(0, true);
    return ok({ value, bytesRead: 4 });
  }
  function tryDecodeF64(data, offset = 0) {
    if (offset < 0 || offset + 8 > data.length) {
      return err(new NumberDecodeError("Not enough bytes for f64"));
    }
    const bytes2 = data.slice(offset, offset + 8);
    const view = new DataView(bytes2.buffer, bytes2.byteOffset, 8);
    const value = view.getFloat64(0, true);
    return ok({ value, bytesRead: 8 });
  }
  var NumberEncodeError = class extends Error {
    constructor(message) {
      super(message);
      this.name = "NumberEncodeError";
    }
  };
  function tryEncodeU8(value) {
  ...
  function tryDecodeChar(data, offset = 0) {
  ...
  var StringEncodeError = class extends Error {
    constructor(message) {
      super(message);
      this.name = "StringEncodeError";
    }
  };
  function tryEncodeString(value) {
    const encoder = new TextEncoder();
  ...
  var BytesDecodeError = class extends Error {
    constructor(message) {
      super(message);
      this.name = "BytesDecodeError";
    }
  };
  function tryDecodeBytes(data, offset = 0) {
    if (offset < 0 || offset >= data.length) {
  ...
  var BytesEncodeError = class extends Error {
    constructor(message) {
      super(message);
      this.name = "BytesEncodeError";
    }
  };
  function tryEncodeBytes(value) {
  ...
  function trySerialize(schema, value) {
    switch (schema.kind) {
      case "bool":
        return serializeBool(schema, value);
      case "i8":
        return serializeI8(schema, value);
      case "i16":
        return serializeI16(schema, value);
      case "i32":
        return serializeI32(schema, value);
      case "i64":
        return serializeI64(schema, value);
      case "i128":
        return serializeI128(schema, value);
      case "u8":
        return serializeU8(schema, value);
      case "u16":
        return serializeU16(schema, value);
      case "u32":
        return serializeU32(schema, value);
      case "u64":
        return serializeU64(schema, value);
      case "u128":
        return serializeU128(schema, value);
      case "f32":
        return serializeF32(schema, value);
      case "f64":
        return serializeF64(schema, value);
      case "char":
        return serializeChar(schema, value);
      case "string":
        return serializeString(schema, value);
      case "bytes":
        return serializeBytes(schema, value);
      case "option":
        return serializeOption(schema, value);
      case "unit":
        return serializeUnit();
      case "unit_struct":
        return serializeUnitStruct();
      case "newtype_struct":
        return serializeNewtypeStruct(schema, value);
      case "seq":
        return serializeSeq(schema, value);
      case "tuple":
        return serializeTuple(schema, value);
      case "tuple_struct":
        return serializeTupleStruct(schema, value);
      case "map":
        return serializeMap(schema, value);
      case "struct":
        return serializeStruct(schema, value);
      case "enum":
        return serializeEnum(schema, value);
      default: {
        const _exhaustive = schema;
        return err(new SerializeError(`Unknown schema kind: ${_exhaustive.kind}`));
      }
    }
  }
  function serializeBool(_schema, value) {
  ...
  function serializeI8(_schema, value) {
  ...
  function serializeI16(_schema, value) {
  ...
  function serializeI32(_schema, value) {
  ...
  function serializeI64(_schema, value) {
  ...
  function serializeI128(_schema, value) {
  ...
  function serializeU8(_schema, value) {
  ...
  function serializeU16(_schema, value) {
  ...
  function serializeU32(_schema, value) {
  ...
  function serializeU64(_schema, value) {
  ...
  function serializeU128(_schema, value) {
  ...
  function serializeF32(_schema, value) {
  ...
  function serializeF64(_schema, value) {
  ...
  function serializeChar(_schema, value) {
  ...
  function serializeString(_schema, value) {
  ...
  function serializeBytes(_schema, value) {
  ...
  function serializeOption(schema, value) {
    if (value === null) {
  ...
  function serializeUnit(_schema, _value) {
    return ok(new Uint8Array(0));
  }
  function serializeUnitStruct(_schema, _value) {
    return ok(new Uint8Array(0));
  }
  function serializeNewtypeStruct(schema, value) {
    const innerResult = trySerialize(schema.inner, value);
    if (!innerResult.ok) {
  ...
  function serializeSeq(schema, value) {
    ...
    const parts = [lengthResult.value.bytes];
  ...
  function serializeTuple(schema, value) {
    const parts = [];
    let totalSize = 0;
  ...
  function serializeTupleStruct(schema, value) {
    const parts = [];
    let totalSize = 0;
    for (let i = 0; i < schema.items.length; i++) {
      const itemSchema = schema.items[i];
  ...
  function serializeMap(schema, value) {
    ...
    const parts = [countResult.value.bytes];
    let totalSize = countResult.value.bytesWritten;
    let i = 0;
    for (const [key2, val] of value.entries()) {
      const keyResult = trySerialize(schema.key, key2);
      if (!keyResult.ok) {
  ...
  function serializeStruct(schema, value) {
    const parts = [];
    let totalSize = 0;
  ...
  function serializeEnum(schema, value) {
    ...
    switch (variant.kind) {
      ...
      case "tuple_variant": {
        const variantValue = value.value;
        const parts = [discriminantResult.value.bytes];
      ...
      default: {
        ...
        return err(new SerializeError(`Unknown variant kind: ${_exhaustive.kind}`));
      }
    }
  }
  ...
  function tryDeserialize(schema, data, offset = 0) {
    if (offset < 0 || offset > data.length) {
      return err(new DeserializeError("Offset out of bounds"));
    }
    switch (schema.kind) {
      case "bool":
        return deserializeBool(schema, data, offset);
      case "i8":
        return deserializeI8(schema, data, offset);
      case "i16":
        return deserializeI16(schema, data, offset);
      case "i32":
        return deserializeI32(schema, data, offset);
      case "i64":
        return deserializeI64(schema, data, offset);
      case "i128":
        return deserializeI128(schema, data, offset);
      case "u8":
        return deserializeU8(schema, data, offset);
      case "u16":
        return deserializeU16(schema, data, offset);
      case "u32":
        return deserializeU32(schema, data, offset);
      case "u64":
        return deserializeU64(schema, data, offset);
      case "u128":
        return deserializeU128(schema, data, offset);
      case "f32":
        return deserializeF32(schema, data, offset);
      case "f64":
        return deserializeF64(schema, data, offset);
      case "char":
        return deserializeChar(schema, data, offset);
      case "string":
        return deserializeString(schema, data, offset);
      case "bytes":
        return deserializeBytes(schema, data, offset);
      case "option":
        return deserializeOption(schema, data, offset);
      case "unit":
        return deserializeUnit();
      case "unit_struct":
        return deserializeUnitStruct();
      case "newtype_struct":
        return deserializeNewtypeStruct(schema, data, offset);
      case "seq":
        return deserializeSeq(schema, data, offset);
      case "tuple":
        return deserializeTuple(schema, data, offset);
      case "tuple_struct":
        return deserializeTupleStruct(schema, data, offset);
      case "map":
        return deserializeMap(schema, data, offset);
      case "struct":
        return deserializeStruct(schema, data, offset);
      case "enum":
        return deserializeEnum(schema, data, offset);
      default: {
        const _exhaustive = schema;
        return err(new DeserializeError(`Unknown schema kind: ${_exhaustive.kind}`));
      }
    }
  }
  function deserializeBool(_schema, data, offset) {
  ...
  function deserializeI8(_schema, data, offset) {
  ...
  function deserializeI16(_schema, data, offset) {
  ...
  function deserializeI32(_schema, data, offset) {
  ...
  function deserializeI64(_schema, data, offset) {
  ...
  function deserializeI128(_schema, data, offset) {
  ...
  function deserializeU8(_schema, data, offset) {
  ...
  function deserializeU16(_schema, data, offset) {
    const result = tryDecodeVarintU16(data, offset);
    if (!result.ok) {
  ...
  function deserializeU32(_schema, data, offset) {
    const result = tryDecodeVarintU32(data, offset);
    if (!result.ok) {
  ...
  function deserializeU64(_schema, data, offset) {
    const result = tryDecodeVarintU64(data, offset);
    if (!result.ok) {
  ...
  function deserializeU128(_schema, data, offset) {
    const result = tryDecodeVarintU128(data, offset);
    if (!result.ok) {
  ...
  function deserializeF32(_schema, data, offset) {
  ...
  function deserializeF64(_schema, data, offset) {
  ...
  function deserializeChar(_schema, data, offset) {
  ...
  function deserializeString(_schema, data, offset) {
  ...
  function deserializeBytes(_schema, data, offset) {
  ...
  function deserializeOption(schema, data, offset) {
  ...
  function deserializeUnit(_schema, _data, _offset) {
    return ok({ value: void 0, bytesRead: 0 });
  }
  function deserializeUnitStruct(_schema, _data, _offset) {
    return ok({ value: {}, bytesRead: 0 });
  }
  function deserializeNewtypeStruct(schema, data, offset) {
  ...
  function deserializeSeq(schema, data, offset) {
    const lengthResult = tryDecodeVarintU64(data, offset);
    if (!lengthResult.ok) {
  ...
  function deserializeTuple(schema, data, offset) {
  ...
  function deserializeTupleStruct(schema, data, offset) {
    let currentOffset = offset;
    const items = [];
    for (let i = 0; i < schema.items.length; i++) {
      const itemSchema = schema.items[i];
  ...
  function deserializeMap(schema, data, offset) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      map2.set(keyResult.value.value, valueResult.value.value);
    }
    return ok({
      value: map2,
      bytesRead: currentOffset - offset
  ...
  function deserializeStruct(schema, data, offset) {
    let currentOffset = offset;
    const obj = {};
    for (const [fieldName, fieldSchema] of Object.entries(schema.fields)) {
  ...
  function deserializeEnum(schema, data, offset) {
    ...
    switch (variant.kind) {
      ...
      case "newtype_variant": {
        ...
        return ok({
          ...
          value: { type: variantName, value: valueResult.value.value },
          bytesRead: currentOffset - offset
        });
      }
      case "tuple_variant": {
        ...
        return ok({
          value: { type: variantName, value: items },
          bytesRead: currentOffset - offset
        });
      }
      case "struct_variant": {
        ...
        return ok({
          value: { type: variantName, value: obj },
          bytesRead: currentOffset - offset
        });
      }
      default: {
        const _exhaustive = variant;
        return err(new DeserializeError(`Unknown variant kind: ${_exhaustive.kind}`));
      }
    }
  }
  function deserialize(schema, data, offset) {
    return unwrap(tryDeserialize(schema, data, offset));
  }

  // parsers/aidoku/wasm-host/imports/defaults.ts
  function hydrateWasmDefaults(env, settings) {
    for (const [key2, raw] of Object.entries(settings)) {
      const value = toDefaultValue(raw);
      if (value) {
        env.defaults.set(key2, value);
      }
    }
  }
  function toDefaultValue(raw) {
  ...
  function createDefaultsImports(env) {
    return {
      get: (keyPtr, len) => {
        try {
          const key2 = env.readString(keyPtr, len);
          const value = env.defaults.get(key2);
          if (!value) return -2 /* InvalidValue */;
          return storeDefault(env, value);
        } catch {
          return -1 /* InvalidKey */;
        }
      },
      set: (keyPtr, len, kind, valuePtr) => {
        try {
          const key2 = env.readString(keyPtr, len);
          if (kind > 6) return -2 /* InvalidValue */;
          const bytes = env.readItemBytes(valuePtr);
          const value = decodeDefault(kind, bytes);
          if (!value) return -4 /* FailedDecoding */;
          env.defaults.set(key2, value);
          return 0 /* Success */;
        } catch {
          return -1 /* InvalidKey */;
  ...
  function storeDefault(env, value) {
    switch (value.kind) {
      case "data":
      ...
      case "stringArray":
        return env.store.storeEncoded(serialize({ kind: "seq", item: string() }, value.value));
      default:
        return -2 /* InvalidValue */;
    }
  }
  function decodeDefault(kind, bytes) {
    try {
      switch (kind) {
        case 0:
        ...
        case 5:
          return { kind: "stringArray", value: deserialize({ kind: "seq", item: string() }, bytes).value };
        case 6:
          return { kind: "null" };
        default:
          return null;
  ...
  // parsers/shared/manga-description.ts
  function parseMangaDescription(description) {
    ...
    for (const block of blocks) {
      ...
      if (block.startsWith("\u0410\u043B\u044C\u0442\u0435\u0440\u043D\u0430\u0442\u0438\u0432\u043D\u044B\u0435 \u043D\u0430\u0437\u0432\u0430\u043D\u0438\u044F:")) {
        const names = block.replace(/^Альтернативные названия:<br\/?>/i, "").split(/<br\/?>|,\s*/).map((name) => name.trim()).filter(Boolean);
        altTitles = names;
        altTitle = names[0];
        continue;
      }
  ...
  var STUB = -1;
  function createCanvasImports(_env) {
    return {
      new_context: () => STUB,
      set_transform: () => STUB,
      draw_image: () => STUB,
  ...
  // parsers/aidoku/wasm-host/schemas.ts
  var MangaStatusSchema = enumType("MangaStatus", {
  ...
  var ContentRatingSchema = enumType("ContentRating", {
    Unknown: unitVariant("Unknown"),
    Safe: unitVariant("Safe"),
    Suggestive: unitVariant("Suggestive"),
    NSFW: unitVariant("NSFW")
  });
  var ViewerSchema = enumType("Viewer", {
    Unknown: unitVariant("Unknown"),
  ...
  var UpdateStrategySchema = enumType("UpdateStrategy", {
    Always: unitVariant("Always"),
    Never: unitVariant("Never")
  });
  var ChapterSchema = struct({
    key: string(),
    title: option(string()),
  ...
  var MangaPageResultSchema = struct({
    entries: seq(MangaSchema),
    has_next_page: bool()
  });
  var FilterValueSchema = enumType("FilterValue", {
  ...
  var HomeComponentSchema = struct({
    title: option(string()),
    subtitle: option(string()),
    value: HomeComponentValueSchema
  });
  var HomeLayoutSchema = struct({
    components: seq(HomeComponentSchema)
  });
  var HomePartialResultSchema = enumType("HomePartialResult", {
  ...
  var FilterValueListSchema = seq(FilterValueSchema);
  var ListingListSchema = seq(ListingSchema);
  var PageListSchema = seq(PageSchema);

  // parsers/aidoku/wasm-host/partial-results.ts
  function decodePostcard(schema, bytes) {
    try {
      return deserialize(schema, bytes).value;
    } catch {
      return null;
    }
  }
  function mergeHomeComponent(layout, component) {
  ...
  function resetPartialResults(env) {
    env.homePartialLayout = null;
    env.partialMangaResults = [];
  }
  function handlePartialResultBytes(env, bytes) {
    const homePartial = decodePostcard(HomePartialResultSchema, bytes);
    if (homePartial) {
      if (homePartial.type === "Layout") {
        env.homePartialLayout = homePartial.value;
        return;
      }
      if (homePartial.type === "Component") {
        ...
        mergeHomeComponent(env.homePartialLayout, homePartial.value);
        return;
      }
    }
    const layoutOnly = decodePostcard(HomeLayoutSchema, bytes);
  ...
  function resolveHomeLayout(env, finalLayout) {
    if (!env.homePartialLayout) {
      return finalLayout;
    }
  ...
  // parsers/aidoku/wasm-host/imports/env.ts
  function createEnvImports(env) {
    return {
      abort: () => {
      ...
      sleep: (seconds) => {
        ...
        while (Date.now() - start < seconds * 1e3) {
        }
      },
  ...
  function getElementsByTagName(tagName20, nodes, recurse = true, limit = Infinity) {
    return filter(Checks["tag_name"](tagName20), nodes, recurse, limit);
  }
  function getElementsByClassName(className2, nodes, recurse = true, limit = Infinity) {
    return filter(getAttribCheck("class", className2), nodes, recurse, limit);
  }
  ...
  function getOneElement(tagName20, node) {
    return getElementsByTagName(tagName20, node, true, 1)[0];
  }
  function fetch(tagName20, where, recurse = false) {
    return textContent(getElementsByTagName(tagName20, where, recurse, 1)).trim();
  }
  function addConditionally(obj, prop2, tagName20, where, recurse = false) {
    const val = fetch(tagName20, where, recurse);
    if (val)
      obj[prop2] = val;
  }
  ...
  var moCallback = (element, parentNode2) => {
    const { ownerDocument } = element;
    const { active, observers } = ownerDocument[MUTATION_OBSERVER];
    if (active) {
      for (const observer of observers) {
        for (const [target, { subtree, childList, characterData }] of observer.nodes) {
          if (childList) {
            if (parentNode2 && (target === parentNode2 || /* c8 ignore next */
            subtree && target.contains(parentNode2)) || !parentNode2 && (subtree && (target === ownerDocument || /* c8 ignore next */
  ...
  var getParentNodeCount = ({ parentNode: parentNode2 }) => {
    let count = 0;
    while (parentNode2) {
      count++;
    ...
    compareDocumentPosition(target) {
      ...
      if (this !== target) {
        ...
        if (self2 < other) {
          ...
          const { childNodes: childNodes2 } = this.parentNode;
          if (childNodes2.indexOf(this) < childNodes2.indexOf(target))
            result += DOCUMENT_POSITION_FOLLOWING;
          else
            result += DOCUMENT_POSITION_PRECEDING;
        }
  ...
  var isConnected = ({ ownerDocument, parentNode: parentNode2 }) => {
    while (parentNode2) {
      if (parentNode2 === ownerDocument)
        return true;
      parentNode2 = parentNode2.parentNode || parentNode2.host;
    }
    return false;
  };
  var parentElement = ({ parentNode: parentNode2 }) => {
    if (parentNode2) {
      switch (parentNode2.nodeType) {
        case DOCUMENT_NODE:
        case DOCUMENT_FRAGMENT_NODE:
          return null;
      }
  ...
  var before = (node, nodes) => {
    const { ownerDocument, parentNode: parentNode2 } = node;
    if (parentNode2)
  ...
  var remove = (prev, current, next) => {
    const { parentNode: parentNode2, nodeType } = current;
    if (prev || next) {
    ...
    if (parentNode2) {
      ...
      moCallback(current, parentNode2);
  ...
  function getElementsByTagName2(tagName20, nodes, recurse = true, limit = Number.POSITIVE_INFINITY) {
    return filter2(Checks2["tag_name"](tagName20), nodes, recurse, limit);
  }
  function getElementsByClassName2(className2, nodes, recurse = true, limit = Number.POSITIVE_INFINITY) {
    return filter2(getAttribCheck2("class", className2), nodes, recurse, limit);
  }
  ...
  function getOneElement2(tagName20, node) {
    return getElementsByTagName2(tagName20, node, true, 1)[0];
  }
  function fetch2(tagName20, where, recurse = false) {
    return textContent2(getElementsByTagName2(tagName20, where, recurse, 1)).trim();
  }
  function addConditionally2(object, property, tagName20, where, recurse = false) {
    const value = fetch2(tagName20, where, recurse);
    if (value)
  ...
  var getChildren3 = ({ childNodes: childNodes2 }) => childNodes2;
  var getName3 = (element) => {
    const { localName } = element;
    return ignoreCase(element) ? localName.toLowerCase() : localName;
  };
  var getParent3 = ({ parentNode: parentNode2 }) => parentNode2;
  var getSiblings3 = (element) => {
    const { parentNode: parentNode2 } = element;
    return parentNode2 ? getChildren3(parentNode2) : element;
  };
  var getText3 = (node) => {
    if (isArray(node))
  ...
  var removeSubsets3 = (nodes) => {
    ...
    while (length--) {
      ...
      for (let { parentNode: parentNode2 } = node; parentNode2; parentNode2 = parentNode2.parentNode) {
        if (nodes.includes(parentNode2)) {
          nodes.splice(length, 1);
          break;
        }
      }
  ...
  var insert = (parentNode2, child, nodes) => {
    const { ownerDocument } = parentNode2;
    for (const node of nodes)
      parentNode2.insertBefore(
  ...
  var ParentNode = class extends Node2 {
    ...
    get childNodes() {
      const childNodes2 = new NodeList();
      let { firstChild } = this;
      while (firstChild) {
        childNodes2.push(firstChild);
    ...
    getElementsByClassName(className2) {
      const elements = new NodeList();
      let { [NEXT]: next, [END]: end } = this;
      while (next !== end) {
        if (next.nodeType === ELEMENT_NODE && next.hasAttribute("class") && next.classList.has(className2))
          elements.push(next);
        next = next[NEXT];
      }
      return elements;
    }
    getElementsByTagName(tagName20) {
      const elements = new NodeList();
      let { [NEXT]: next, [END]: end } = this;
      while (next !== end) {
        if (next.nodeType === ELEMENT_NODE && (next.localName === tagName20 || localCase(next) === tagName20))
          elements.push(next);
        next = next[NEXT];
      }
      return elements;
    ...
    contains(node) {
      ...
      while (parentNode2 && parentNode2 !== this)
        parentNode2 = parentNode2.parentNode;
      return parentNode2 === this;
    }
    insertBefore(node, before2 = null) {
      ...
      switch (node.nodeType) {
        case ELEMENT_NODE:
          ...
          break;
        case DOCUMENT_FRAGMENT_NODE: {
          let { [PRIVATE]: parentNode2, firstChild, lastChild } = node;
          if (firstChild) {
            knownSegment(next[PREV], firstChild, lastChild, next);
            knownAdjacent(node, node[END]);
            if (parentNode2)
              parentNode2.replaceChildren();
            do {
  ...
  var NonElementParentNode = class extends ParentNode {
    ...
    toString() {
      const { childNodes: childNodes2, localName } = this;
      return `<${localName}>${childNodes2.join("")}</${localName}>`;
    }
    toJSON() {
      const json = [];
  ...
  var setInnerHtml = (node, html) => {
    ...
    const { childNodes: childNodes2 } = parseFromString(document, ignoreCase(node), html);
    node.replaceChildren(...childNodes2.map(setOwnerDocument, ownerDocument));
  };
  ...
  var Element2 = class extends ParentNode {
    ...
    cloneNode(deep = false) {
      ...
      const addNext = (next2) => {
        next2.parentNode = parentNode2;
        knownAdjacent($next, next2);
        $next = next2;
      };
      const clone = create2(ownerDocument, this, localName);
      let parentNode2 = clone, $next = clone;
      let { [NEXT]: next, [END]: prev } = this;
      while (next !== prev && (deep || next.nodeType === ATTRIBUTE_NODE)) {
        switch (next.nodeType) {
          case NODE_END:
            knownAdjacent($next, parentNode2[END]);
            $next = parentNode2[END];
            parentNode2 = parentNode2.parentNode;
            break;
          case ELEMENT_NODE: {
          ...
          case ATTRIBUTE_NODE: {
            ...
            attr2.ownerElement = parentNode2;
            addNext(attr2);
            break;
          }
          case TEXT_NODE:
  ...
  var SVGElement = class extends Element2 {
    ...
    setAttribute(name, value) {
      if (name === "class")
        ...
        const { className: className2 } = this;
        className2.baseVal = className2.animVal = value;
      }
  ...
  var HTMLOptionElement = class extends HTMLElement {
    ...
    set selected(value) {
      const option2 = this.parentElement?.querySelector("option[selected]");
      if (option2 && option2 !== this)
  ...
  var Range = class _Range {
    ...
    surroundContents(parentNode2) {
      parentNode2.replaceChildren(this.extractContents());
    }
    setStartBefore(node) {
      this[START] = node;
    }
    ...
    createContextualFragment(html) {
      ...
      if (isSVG) {
        const childNodes2 = [...content.childNodes];
  ...
  var Document2 = class extends NonElementParentNode {
    ...
    cloneNode(deep = false) {
      ...
      if (deep) {
        ...
        const { childNodes: childNodes2 } = this;
        for (let { length } = childNodes2, i = 0; i < length; i++)
          document.insertBefore(childNodes2[i].cloneNode(true), end);
        if (doctype)
          document[DOCTYPE] = childNodes2[0];
      }
      return document;
    }
  ...
  // parsers/aidoku/wasm-host/html-dom.ts
  function createDocument(html, baseUri) {
    const { document } = parseHTML(html);
    return { document, baseUri };
  }
  function createFragment(html, baseUri) {
  ...
  function selectElements(root, selector) {
    const scope = "node" in root ? root.node : root.document;
    const elements = scope.querySelectorAll(selector);
    return Array.from(elements).map((element) => ({
      document: "document" in root && !("node" in root) ? root.document : root.document,
      node: element,
      baseUri: root.baseUri
    }));
  }
  function selectFirstElement(root, selector) {
    const scope = "node" in root ? root.node : root.document;
    const element = scope.querySelector(selector);
    if (!element) return null;
    return {
      document: "document" in root ? root.document : root.document,
      node: element,
      baseUri: "baseUri" in root ? root.baseUri : root.baseUri
    };
  }
  ...
  function elementText(element, trimmed) {
  ...
  function tagName19(element) {
    return element.node.tagName.toLowerCase();
  }
  function className(element) {
    return element.node.className ?? "";
  }
  function hasClass(element, className2) {
    return element.node.classList.contains(className2);
  }
  ...
  // parsers/aidoku/wasm-host/imports/html.ts
  function createHtmlImports(env) {
    return {
      parse: (htmlPtr, htmlLen, baseUrlPtr, baseUrlLen) => {
        try {
          const text = env.readString(htmlPtr, htmlLen);
          const baseUrl = env.readString(baseUrlPtr, baseUrlLen);
          const document = createDocument(text, baseUrl || void 0);
          return env.store.store({ kind: "htmlDocument", value: document });
        } catch {
          return -2 /* InvalidString */;
        }
      },
      parse_fragment: (htmlPtr, htmlLen, baseUrlPtr, baseUrlLen) => {
        try {
          const text = env.readString(htmlPtr, htmlLen);
          const baseUrl = env.readString(baseUrlPtr, baseUrlLen);
          const document = createFragment(text, baseUrl || void 0);
          return env.store.store({ kind: "htmlDocument", value: document });
        } catch {
          return -2 /* InvalidString */;
        }
      },
      escape: (textPtr, textLen) => {
        try {
          const text = env.readString(textPtr, textLen);
          return env.store.store({ kind: "string", value: escapeHtml(text) });
        } catch {
          return -2 /* InvalidString */;
        }
      },
      unescape: (textPtr, textLen) => {
        try {
          const text = env.readString(textPtr, textLen);
          return env.store.store({ kind: "string", value: unescapeHtml(text) });
        } catch {
          return -2 /* InvalidString */;
        }
      },
      kind: (rid) => {
        ...
        if (!item) return -1 /* InvalidDescriptor */;
        if (item.kind === "htmlElement") {
          return isElement(item.value) ? 5 /* Element */ : 1 /* Node */;
        }
        if (item.kind === "htmlNode") {
          if (isElement(item.value)) return 5 /* Element */;
          if (isComment3(item.value)) return 4 /* Comment */;
          if (isText3(item.value)) return 2 /* TextNode */;
          if (isDocument2(item.value)) return 7 /* Document */;
          return 1 /* Node */;
        }
        if (item.kind === "htmlDocument") return 7 /* Document */;
        if (item.kind === "htmlElementList") return 6 /* ElementList */;
        return 0 /* Unknown */;
      },
      child_nodes: (rid) => {
        const item = env.store.getMut(rid);
        if (item?.kind === "htmlNode" || item?.kind === "htmlElement") {
          const nodes = childNodes(item.value);
          return env.store.store({ kind: "htmlNodeList", value: nodes });
        }
        return -1 /* InvalidDescriptor */;
      },
      has_attr: (rid, attrPtr, attrLen) => {
        try {
          const attr2 = env.readString(attrPtr, attrLen);
          const item = env.store.getMut(rid);
          if (item?.kind !== "htmlElement") return -1 /* InvalidDescriptor */;
          return hasAttr(item.value, attr2) ? 1 : 0;
        } catch {
          return -2 /* InvalidString */;
        }
      },
      set_attr: () => -1 /* InvalidDescriptor */,
      remove_attr: () => -1 /* InvalidDescriptor */,
      set_text: () => -1 /* InvalidDescriptor */,
      set_html: () => -1 /* InvalidDescriptor */,
      prepend: () => -1 /* InvalidDescriptor */,
      append: () => -1 /* InvalidDescriptor */,
      children: (rid) => {
        const item = env.store.getMut(rid);
        if (item?.kind !== "htmlElement") return -1 /* InvalidDescriptor */;
        const children = elementChildNodes(item.value);
        if (children.length === 0) return -5 /* NoResult */;
        return env.store.store({ kind: "htmlElementList", value: children });
      },
      base_uri: (rid) => {
        const item = env.store.getMut(rid);
        if (item?.kind !== "htmlElement") return -1 /* InvalidDescriptor */;
        if (!item.value.baseUri) return -5 /* NoResult */;
        return env.store.store({ kind: "string", value: item.value.baseUri });
      },
      own_text: (rid) => {
        const item = env.store.getMut(rid);
        if (item?.kind !== "htmlElement") return -1 /* InvalidDescriptor */;
        const text = ownText(item.value);
        if (text == null) return -5 /* NoResult */;
        return env.store.store({ kind: "string", value: text });
      },
      data: () => -1 /* InvalidDescriptor */,
      id: (rid) => {
        const item = env.store.getMut(rid);
        if (item?.kind !== "htmlElement") return -1 /* InvalidDescriptor */;
        const id = elementId(item.value);
        if (!id) return -5 /* NoResult */;
        return env.store.store({ kind: "string", value: id });
      },
      tag_name: (rid) => {
        const item = env.store.getMut(rid);
        if (item?.kind !== "htmlElement") return -1 /* InvalidDescriptor */;
        return env.store.store({ kind: "string", value: tagName19(item.value) });
      },
      class_name: (rid) => {
        const item = env.store.getMut(rid);
        if (item?.kind !== "htmlElement") return -1 /* InvalidDescriptor */;
        return env.store.store({ kind: "string", value: className(item.value) });
      },
      has_class: (rid, classPtr, classLen) => {
        try {
          const className2 = env.readString(classPtr, classLen);
          const item = env.store.getMut(rid);
          if (item?.kind !== "htmlElement") return -1 /* InvalidDescriptor */;
          return hasClass(item.value, className2) ? 1 : 0;
        } catch {
          return -2 /* InvalidString */;
        }
      },
      add_class: () => -1 /* InvalidDescriptor */,
      remove_class: () => -1 /* InvalidDescriptor */,
      parent: (rid) => {
        const item = env.store.getMut(rid);
        if (item?.kind === "htmlElement") {
          const parent = parentNode(item.value);
          if (!parent || !isElement(parent)) return -5 /* NoResult */;
          return env.store.store({ kind: "htmlElement", value: parent });
        }
        if (item?.kind === "htmlNode") {
          const parent = parentNode(item.value);
          if (!parent) return -5 /* NoResult */;
      ...
      siblings: (rid) => {
        const item = env.store.getMut(rid);
        if (item?.kind === "htmlElement") {
          const siblings = elementChildNodes(item.value);
          return env.store.store({ kind: "htmlElementList", value: siblings });
        }
        if (item?.kind === "htmlNode") {
          return env.store.store({ kind: "htmlNodeList", value: siblingNodes(item.value) });
        }
        return -1 /* InvalidDescriptor */;
      },
      next: (rid) => {
        ...
        if (item?.kind === "htmlElement" || item?.kind === "htmlNode") {
          const next = nextNode(item.value);
          if (!next) return -5 /* NoResult */;
          if (item.kind === "htmlElement" && isElement(next)) {
      ...
      previous: (rid) => {
        const item = env.store.getMut(rid);
        if (item?.kind === "htmlElement" || item?.kind === "htmlNode") {
          const prev = previousNode(item.value);
          if (!prev) return -5 /* NoResult */;
          if (item.kind === "htmlElement" && isElement(prev)) {
      ...
      first: (rid) => {
        const item = env.store.getMut(rid);
        if (item?.kind !== "htmlElementList" || item.value.length === 0) return -1 /* InvalidDescriptor */;
      ...
      attr: (rid, keyPtr, keyLen) => {
        try {
          const key2 = env.readString(keyPtr, keyLen);
          const item = env.store.getMut(rid);
          let value = null;
          if (item?.kind === "htmlElement") {
            value = attr(item.value, key2);
          } else if (item?.kind === "htmlElementList") {
            for (const element of item.value) {
              const attr2 = attr(element, key2);
              if (attr2) {
                value = attr2;
                break;
              }
      ...
      outer_html: (rid) => {
        const item = env.store.getMut(rid);
        if (item?.kind === "htmlElement") {
          return env.store.store({ kind: "string", value: outerHtml(item.value) });
        }
        if (item?.kind === "htmlElementList") {
          const html = item.value.map((element) => outerHtml(element)).join("\n");
          return env.store.store({ kind: "string", value: html });
        }
        return -1 /* InvalidDescriptor */;
      },
      remove: () => -1 /* InvalidDescriptor */,
      select: (rid, queryPtr, queryLen) => {
        try {
          const selector = env.readString(queryPtr, queryLen);
          const item = env.store.getMut(rid);
          if (item?.kind === "htmlDocument") {
            const elements = selectElements(item.value, selector);
            return env.store.store({ kind: "htmlElementList", value: elements });
          }
          if (item?.kind === "htmlElement") {
            const elements = selectElements(item.value, selector);
            if (elements.length === 0) return -5 /* NoResult */;
            return env.store.store({ kind: "htmlElementList", value: elements });
          }
          if (item?.kind === "htmlElementList") {
            const elements = item.value.flatMap((element) => selectElements(element, selector));
            if (elements.length === 0) return -5 /* NoResult */;
            return env.store.store({ kind: "htmlElementList", value: elements });
          }
          return -1 /* InvalidDescriptor */;
      ...
      select_first: (rid, queryPtr, queryLen) => {
        try {
          const selector = env.readString(queryPtr, queryLen);
          const item = env.store.getMut(rid);
          if (item?.kind === "htmlDocument") {
            const element = selectFirstElement(item.value, selector);
            if (!element) return -5 /* NoResult */;
            return env.store.store({ kind: "htmlElement", value: element });
          }
          if (item?.kind === "htmlElement") {
            const element = selectFirstElement(item.value, selector);
          ...
          if (item?.kind === "htmlElementList") {
            for (const element of item.value) {
              const found = selectFirstElement(element, selector);
      ...
      text: (rid) => {
        const item = env.store.getMut(rid);
        if (item?.kind === "htmlElement") {
          return env.store.store({ kind: "string", value: elementText(item.value, true) });
        }
        if (item?.kind === "htmlNode") {
          const text = nodeText(item.value);
          if (text == null) return -5 /* NoResult */;
          return env.store.store({ kind: "string", value: text });
        }
        if (item?.kind === "htmlElementList") {
          const text = item.value.map((element) => elementText(element, true)).join(" ");
          return env.store.store({ kind: "string", value: text });
        }
        return -1 /* InvalidDescriptor */;
      },
      untrimmed_text: (rid) => {
        const item = env.store.getMut(rid);
        if (item?.kind === "htmlElement") {
          return env.store.store({ kind: "string", value: elementText(item.value, false) });
        }
        if (item?.kind === "htmlElementList") {
          const text = item.value.map((element) => elementText(element, false)).join(" ");
          return env.store.store({ kind: "string", value: text });
        }
        return -1 /* InvalidDescriptor */;
      },
      html: (rid) => {
        const item = env.store.getMut(rid);
        if (item?.kind === "htmlElement") {
          return env.store.store({ kind: "string", value: innerHtml(item.value) });
        }
        if (item?.kind === "htmlElementList") {
          const html = item.value.map((element) => innerHtml(element)).join("\n");
          return env.store.store({ kind: "string", value: html });
        }
        return -1 /* InvalidDescriptor */;
      }
  ...
  // parsers/aidoku/wasm-host/imports/js.ts
  var STUB2 = -1;
  function createJsImports(_env) {
    return {
      context_create: () => STUB2,
      context_eval: () => STUB2,
      context_eval_async: () => STUB2,
      context_get: () => STUB2,
      webview_create: () => STUB2,
      webview_set_rule_list: () => STUB2,
      webview_load: () => STUB2,
      webview_load_html: () => STUB2,
      webview_wait_for_load: () => STUB2,
      webview_eval: () => STUB2,
      webview_eval_async: () => STUB2,
      webview_add_user_script: () => STUB2,
      webview_get_cookies: () => STUB2,
      webview_delete_cookie: () => STUB2
    };
  }

  // parsers/shared/fetch-headers.ts
  var DEFAULT_FETCH_USER_AGENT = "Mozilla/5.0 (iPhone; CPU iPhone OS 18_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/18.0 Mobile/15E148 Safari/604.1";
  var NHENTAI_FETCH_USER_AGENT = "Mozilla/5.0 (iPhone; CPU iPhone OS 17_2 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) GSA/300.0.598994205 Mobile/15E148 Safari/604";
  function isNhentaiUrl(url) {
    try {
  ...
  function buildFetchHeaders(url, sourceHeaders = {}, sourceBaseUrl) {
    ...
    const hasReferer = Object.keys(headers).some((key2) => key2.toLowerCase() === "referer");
    const referer = hasReferer ? void 0 : resolveReferer(url, sourceBaseUrl);
    if (referer) {
      headers.Referer = referer;
      if (!Object.keys(headers).some((key2) => key2.toLowerCase() === "origin")) {
        headers.Origin = new URL(referer).origin;
      }
    }
    return headers;
  }
  function resolveReferer(url, sourceBaseUrl) {
    if (sourceBaseUrl) {
      try {
  ...
  // parsers/aidoku/wasm-host/imports/net.ts
  var METHODS = ["GET", "POST", "PUT", "HEAD", "DELETE", "PATCH", "OPTIONS", "CONNECT", "TRACE"];
  function createNetImports(env) {
    return {
      init: (method) => {
        const httpMethod = METHODS[method];
        if (!httpMethod) return -3 /* InvalidMethod */;
        const request = {
          method: httpMethod,
          headers: {}
        };
      ...
      send_all: (ridPtr, len) => {
        try {
          ...
          if (rids.length === 0) return 0 /* Success */;
          const prepared = rids.map((rid) => {
            const request = getRequest(env, rid);
            if (!request) return { rid, error: -1 /* InvalidDescriptor */ };
            if (!request.url) return { rid, error: -4 /* InvalidUrl */ };
            return {
              rid,
              fetchRequest: toFetchBridgeRequest(env, rid, request)
            };
          });
          const errors = new Array(rids.length);
          let wasError = false;
          const batch = prepared.filter(
            (item) => "fetchRequest" in item
          );
          ...
          if (batch.length > 0) {
            const responses = batch.length === 1 ? [env.fetchBridge.send(batch[0].fetchRequest)] : env.fetchBridge.sendAll(batch.map((item) => item.fetchRequest));
            batch.forEach((item, index) => {
              const response = responses[index];
              if (!response) {
                const slot2 = rids.indexOf(item.rid);
                errors[slot2] = -10 /* RequestError */;
                wasError = true;
                return;
              }
              ...
              if (!request) {
                const slot2 = rids.indexOf(item.rid);
                errors[slot2] = -1 /* InvalidDescriptor */;
                wasError = true;
                return;
              }
              request.response = {
              ...
              errors[slot] = 0 /* Success */;
            });
          }
          env.writeValues(ridPtr, errors);
          return wasError ? -10 /* RequestError */ : 0 /* Success */;
        } catch {
          return -1 /* InvalidDescriptor */;
        }
      },
      set_url: (rid, ptr, len) => {
        try {
          ...
          new URL(url);
          const request = getRequest(env, rid);
          if (!request) return -1 /* InvalidDescriptor */;
          request.url = url;
          return 0 /* Success */;
        } catch {
          return -4 /* InvalidUrl */;
      ...
      set_header: (rid, keyPtr, keyLen, valPtr, valLen) => {
        try {
          const key2 = env.readString(keyPtr, keyLen);
          const value = env.readString(valPtr, valLen);
          const request = getRequest(env, rid);
          if (!request) return -1 /* InvalidDescriptor */;
          request.headers[key2] = value;
          return 0 /* Success */;
        } catch {
          return -2 /* InvalidString */;
        }
      ...
      data_len: (rid) => {
        const request = getRequest(env, rid);
        if (!request?.response) return -8 /* MissingResponse */;
        return request.response.data.length;
      },
      read_data: (rid, buffer, size) => {
        const request = getRequest(env, rid);
        if (!request?.response) return -8 /* MissingResponse */;
        const data = request.response.data;
        if (size > data.length) return -11 /* FailedMemoryWrite */;
        try {
      ...
      get_header: (rid, keyPtr, keyLen) => {
        try {
          const key2 = env.readString(keyPtr, keyLen).toLowerCase();
          const request = getRequest(env, rid);
          if (!request?.response) return -8 /* MissingResponse */;
          const value = Object.entries(request.response.headers).find(([header]) => header.toLowerCase() === key2)?.[1];
          if (!value) return -7 /* MissingData */;
          return env.store.store({ kind: "string", value });
        } catch {
          return -2 /* InvalidString */;
        }
      ...
      set_rate_limit: (_permits, _period, _unit) => {
      }
    };
  }
  function getRequest(env, rid) {
    return asRequest(env.store.getMut(rid));
  }
  function toFetchBridgeRequest(env, rid, request) {
    ...
    return {
      ...
      url: request.url,
      headers,
      body: request.body ? new TextDecoder().decode(request.body) : void 0,
      timeoutMs: request.timeout ? request.timeout * 1e3 : 6e4,
      sourceBaseUrl: env.sourceBaseUrl,
      sourceId: env.sourceId
    };
  }
  function applyFetchResponse(request, response) {
    request.response = {
      url: response.url,
      status: response.status,
      headers: response.headers ?? {},
  ...
  function commonSend(env, rid) {
    const request = getRequest(env, rid);
    if (!request) return -1 /* InvalidDescriptor */;
    if (!request.url) return -4 /* InvalidUrl */;
    try {
  ...
  // parsers/aidoku/wasm-host/imports/std.ts
  function createStdImports(env) {
    return {
      print: (ptr, len) => {
        try {
          const text = env.readString(ptr, len);
...
`);
      ...
      buffer_len: (rid) => {
        ...
        if (!data) return -1 /* InvalidDescriptor */;
        return data.length;
      },
      read_buffer: (rid, ptr, size) => {
        const data = asEncoded(env.store.get(rid));
        if (!data) return -1 /* InvalidDescriptor */;
        if (size > data.length) return -3 /* FailedMemoryWrite */;
        try {
      ...
      current_date: () => Date.now() / 1e3,
      utc_offset: () => (/* @__PURE__ */ new Date()).getTimezoneOffset() * 60,
      parse_date: (datePtr, dateLen, formatPtr, formatLen, _localePtr, _localeLen, timezonePtr, timezoneLen) => {
        try {
          ...
          if (timestamp == null) return -5 /* InvalidDateString */;
          return timestamp;
        } catch {
          return -4 /* InvalidString */;
        }
      }
    };
  }
  function swiftDateFormatToChrono(format) {
    let result = "";
    let i = 0;
  ...
  function parseWithFormat(dateStr, format, timezone) {
    const parts = tokenize(dateStr, format);
    if (!parts) return null;
    const year = parts.year ?? (/* @__PURE__ */ new Date()).getFullYear();
    ...
    if (timezone === "current") {
      return Math.floor(date.getTime() / 1e3) - (/* @__PURE__ */ new Date()).getTimezoneOffset() * 60;
    }
    return Math.floor(date.getTime() / 1e3);
  }
  function tokenize(dateStr, format) {
    const values = {};
    let i = 0;
    let j = 0;
  ...
  function readChunk(input, start, token) {
    if (token === "Y") {
    ...
    if (token === "y") {
      ...
      return /^\d{2}$/.test(raw) ? { raw, value: 2e3 + Number(raw) } : null;
    }
  ...
  function storeEncodedValue(env, schema, value) {
    const bytes = serialize(schema, value);
    return env.store.storeEncoded(bytes);
  }

  // parsers/aidoku/wasm-host/imports/index.ts
  function createWasmImports(env) {
    return {
  ...
  // parsers/aidoku/wasm-host/runner.ts
  var DEFAULT_BIG_SCROLLER_INTERVAL = 5;
  var instanceCache = /* @__PURE__ */ new Map();
  function resetWasmSourceRuntime(sourceId) {
    instanceCache.delete(sourceId);
  }
  async function loadWasmInstance(sourceId, wasm, env) {
    ...
    const wasmInstance = "instance" in instantiated ? instantiated.instance : instantiated;
    env.memory = wasmInstance.exports.memory;
    wasmInstance.exports.start?.();
    instanceCache.set(sourceId, wasmInstance);
    return wasmInstance;
  }
  async function invokeExport(ctx, method, args, providedEnv) {
    const env = providedEnv ?? new WasmEnv(
      ctx.postToParent ? new SyncFetchBridge((message) => ctx.postToParent?.(message)) : {
        send: () => {
  ...
  function encodeArgs(method, args, env) {
    switch (method) {
      case "get_search_manga_list": {
        ...
        const filters2 = (params.filters ?? []).map(toPostcardFilter);
        const filtersRid = storeEncodedValue(env, FilterValueListSchema, filters2);
        return [queryRid, params.page, filtersRid];
      }
      case "get_manga_list": {
        const { listing, page } = args;
  ...
  function callExport(instance, method, args) {
  ...
  function decodeResult(env, instance, method, ptr) {
    if (ptr < 0) {
    ...
    switch (method) {
      case "get_search_manga_list":
      case "get_manga_list":
        return decodePostcard2(MangaPageResultSchema, bytes, method);
      case "get_manga_update":
        return decodePostcard2(MangaSchema, bytes, method);
      case "get_page_list":
        return decodePostcard2(PageListSchema, bytes, method);
      case "get_home":
        return resolveHomeLayout(env, decodePostcard2(HomeLayoutSchema, bytes, method));
      case "get_listings":
        return decodePostcard2(ListingListSchema, bytes, method);
      default:
        return bytes;
    }
  }
  function storePlainString(env, value) {
    return env.store.store({ kind: "string", value });
  }
  function decodePostcard2(schema, bytes, method) {
    try {
      return deserialize(schema, bytes).value;
    } catch (error) {
  ...
  function toPostcardListing(listing) {
    return {
      id: listing.id,
      name: listing.name ?? listing.id,
      kind: listing.kind === "list" ? { type: "List" } : { type: "Default" }
    };
  }
  function toPostcardManga(manga) {
    return {
      key: manga.key,
      title: manga.title,
  ...
  function toPostcardChapter(chapter) {
    return {
      key: chapter.key,
  ...
  function toPostcardFilter(filter3) {
    switch (filter3.type) {
      case "text":
        return { type: "Text", value: { id: filter3.id, value: filter3.value } };
      case "sort":
        return { type: "Sort", value: { id: filter3.id, index: filter3.index, ascending: filter3.ascending } };
      case "select":
        return { type: "Select", value: { id: filter3.id, value: filter3.value } };
      case "multiSelect":
        return {
          type: "MultiSelect",
          value: { id: filter3.id, included: filter3.included, excluded: filter3.excluded ?? [] }
        };
      case "check":
        return { type: "Check", value: { id: filter3.id, value: filter3.value ? 1 : 0 } };
      case "range": {
        let from = filter3.from ?? null;
        if (from === 0 && filter3.id !== "rating") {
          from = null;
        }
        return {
          type: "Range",
          value: { id: filter3.id, from, to: filter3.to ?? null }
        };
      }
      default:
        throw new Error(`Unsupported filter type: ${filter3.type}`);
    }
  }
  ...
  function normalizeResult(method, value) {
    ...
    if (method === "get_manga_update") {
      return fromPostcardManga(value);
    }
    if (method === "get_page_list") {
      return value.map((page) => {
        if (page.content.type === "Text") {
          return { text: postcardTupleString(page.content.value, 0), thumbnail: page.thumbnail ?? void 0 };
        }
        if (page.content.type === "Url") {
          ...
          const headers = context && typeof context === "object" && !Array.isArray(context) ? context : void 0;
          return {
            url,
            thumbnail: page.thumbnail ?? void 0,
        ...
        if (page.content.type === "Zip") {
          return {
            ...
            zipEntry: postcardTupleString(page.content.value, 1) || void 0,
            thumbnail: page.thumbnail ?? void 0
          };
        }
        return { thumbnail: page.thumbnail ?? void 0 };
      });
    }
    if (method === "get_home") {
      return fromPostcardHomeLayout(value);
    }
    if (method === "get_listings") {
      return value.map(fromPostcardListing);
    }
    return value;
  }
  function postcardTupleItem(value, index) {
  ...
  function postcardTupleString(value, index) {
    const item = postcardTupleItem(value, index);
    return typeof item === "string" ? item : "";
  }
  function fromPostcardListing(listing) {
    return {
      id: listing.id,
      name: listing.name,
  ...
  function fromPostcardLink(link) {
    ...
    if (value?.type === "Url") {
      ...
      return { manga, subtitle: link.subtitle ?? void 0 };
    }
    if (link.title) {
      return {
        manga: {
        ...
        subtitle: link.subtitle ?? void 0
      };
    }
    return null;
  }
  function fromPostcardHomeComponent(component) {
    const title = component.title ?? void 0;
    const subtitle = component.subtitle ?? void 0;
    const { value } = component;
    switch (value.type) {
      case "Scroller": {
        const scrollerEntries = value.value.entries.map(fromPostcardLink).filter((entry) => entry != null);
        return {
          title,
          subtitle,
          kind: "scroller",
          ...
          scrollerEntries,
          listing: value.value.listing ? fromPostcardListing(value.value.listing) : void 0
        };
      }
      case "BigScroller":
        return {
      ...
      case "ImageScroller": {
        const scrollerEntries = value.value.links.map(fromPostcardLink).filter((entry) => entry != null);
        return {
          title,
          subtitle,
          kind: "scroller",
      ...
      case "MangaList": {
        const entries2 = value.value.entries.map(fromPostcardLink).filter((entry) => entry != null).map((entry) => entry.manga);
        const pageSize = value.value.page_size ?? void 0;
        return {
          title,
          subtitle,
          kind: "mangaGrid",
          entries: pageSize != null ? entries2.slice(0, pageSize) : entries2,
          listing: value.value.listing ? fromPostcardListing(value.value.listing) : void 0,
          pageSize,
      ...
      case "MangaChapterList":
        return {
          ...
          listing: value.value.listing ? fromPostcardListing(value.value.listing) : void 0,
          pageSize: value.value.page_size ?? void 0
        };
      case "Filters":
        return {
          title,
          ...
          kind: "filters",
          entries: [],
          filterItems: value.value.map(fromPostcardFilterItem).filter((item) => item != null)
        };
      case "Links":
        return {
          title,
          ...
          kind: "links",
          entries: [],
          links: value.value.map(fromPostcardNavigationLink).filter((link) => link != null)
        };
      default:
        return null;
    }
  ...
  function fromPostcardFilterItem(item) {
    ...
    return {
      title: item.title,
      filters: (item.values ?? []).map(fromPostcardFilterValue).filter((value) => value != null)
    };
  }
  function fromPostcardFilterValue(filter3) {
    switch (filter3.type) {
      case "Text":
        return { type: "text", id: String(filter3.value.id), value: String(filter3.value.value ?? "") };
      case "Sort":
        return {
          type: "sort",
          id: String(filter3.value.id),
          index: Number(filter3.value.index ?? 0),
          ascending: Boolean(filter3.value.ascending)
        };
      case "Check":
        return { type: "check", id: String(filter3.value.id), value: Number(filter3.value.value ?? 0) === 1 };
      case "Select":
        return { type: "select", id: String(filter3.value.id), value: String(filter3.value.value ?? "") };
      case "MultiSelect":
        return {
          type: "multiSelect",
          id: String(filter3.value.id),
          included: Array.isArray(filter3.value.included) ? filter3.value.included.map(String) : [],
          excluded: Array.isArray(filter3.value.excluded) ? filter3.value.excluded.map(String) : []
        };
      case "Range":
        return {
          type: "range",
          id: String(filter3.value.id),
          from: filter3.value.from != null ? Number(filter3.value.from) : void 0,
          to: filter3.value.to != null ? Number(filter3.value.to) : void 0
        };
      default:
        return null;
    }
  }
  function fromPostcardNavigationLink(link) {
    const value = link.value;
    const base = {
      title: link.title,
      subtitle: link.subtitle ?? void 0,
      imageUrl: link.image_url ?? void 0
    };
    if (value?.type === "Manga") {
      const manga = fromPostcardManga(value.value);
      if (link.title) manga.title = link.title;
  ...
  function fromPostcardHomeLayout(layout) {
    return {
      components: layout.components.map(fromPostcardHomeComponent).filter((component) => component != null)
    };
  }
  function fromPostcardManga(manga) {
    return {
      key: manga.key,
      title: manga.title,
      cover: manga.cover ?? void 0,
      artists: manga.artists ?? void 0,
      authors: manga.authors ?? void 0,
      description: manga.description ?? void 0,
      url: manga.url ?? void 0,
      tags: manga.tags ?? void 0,
  ...
  function fromPostcardChapter(chapter) {
    return {
      ...
      language: chapter.language ?? void 0,
      thumbnail: chapter.thumbnail ?? void 0,
  ...
  function fromContentRating(rating) {
    switch (rating.type) {
      case "Safe":
        return "safe";
      case "Suggestive":
  ...
  function createHost(options) {
    ...
    const wasmCache = /* @__PURE__ */ new Map();
