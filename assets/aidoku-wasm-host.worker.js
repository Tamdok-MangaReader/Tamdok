...
(() => {
  ...
  var SyncFetchBridge = class {
    ...
    send(request) {
      const id = request.id ?? `fetch-${++this.counter}`;
      const fullRequest = { ...request, id };
      if (isRnWebView()) {
        return sendWithSyncXhr(fullRequest);
      }
      if (typeof SharedArrayBuffer !== "undefined") {
        try {
          return this.sendWithAtomics(fullRequest);
        } catch {
        }
      }
      return sendWithSyncXhr(fullRequest);
  ...
  function createWorkerFetchBridge() {
    let canUseSharedArrayBuffer = false;
    try {
      canUseSharedArrayBuffer = typeof SharedArrayBuffer !== "undefined" && new SharedArrayBuffer(4).byteLength === 4;
    } catch {
  ...
  function createWorkerSharedArrayBufferFetchBridge() {
    let counter = 0;
  ...
  var WasmEnv = class {
    ...
    readBytes(ptr, len) {
      ...
      const view = this.memoryView();
      return new Uint8Array(view.buffer, view.byteOffset + ptr, len);
    }
    readValues(ptr, len) {
      const view = this.memoryView();
      const values = [];
  ...
  var DEFAULT_USER_AGENT = "Aidoku/1 CFNetwork/3826.500.131 Darwin/24.5.0";
    ...
    return {
      ...
      send_all: (ridPtr, len) => {
        try {
          ...
          let wasError = false;
          const errors = rids.map((rid) => {
            const result = commonSend(env, rid);
            if (result !== 0 /* Success */) wasError = true;
            return result;
          });
          env.writeValues(ridPtr, errors);
          if (wasError) return -10 /* RequestError */;
          return 0 /* Success */;
        } catch {
          return -1 /* InvalidDescriptor */;
        }
  ...
  function commonSend(env, rid) {
    ...
    if (!Object.keys(headers).some((key2) => key2.toLowerCase() === "user-agent")) {
      headers["User-Agent"] = DEFAULT_USER_AGENT;
    }
    try {
      const response = env.fetchBridge.send({
        id: `net-${rid}`,
        method: request.method,
        url: request.url,
        headers,
...
`);
  ...
  function encodeArgs(method, args, env) {
    switch (method) {
      case "get_search_manga_list": {
        const params = args;
        const queryRid = storeEncodedValue(env, StringSchema, params.query ?? "");
  ...
  function decodeResult(env, instance, method, ptr) {
    ...
    switch (method) {
      case "get_search_manga_list":
      case "get_manga_list":
        return deserialize(MangaPageResultSchema, bytes).value;
      case "get_manga_update":
        return deserialize(MangaSchema, bytes).value;
      case "get_page_list":
        return deserialize(PageListSchema, bytes).value;
      case "get_home":
        return deserialize(HomeLayoutSchema, bytes).value;
      case "get_listings":
        return deserialize(ListingListSchema, bytes).value;
      default:
        return bytes;
    }
  }
    ...
    if (method === "get_page_list") {
      return value.map((page) => {
        if (page.content.type === "Text") {
          return { text: page.content.value[0], thumbnail: page.thumbnail ?? void 0 };
        }
        if (page.content.type === "Url") {
          const tuple = page.content.value;
          return { url: tuple[0], thumbnail: page.thumbnail ?? void 0 };
        }
  ...
  function fromPostcardLink(link) {
    ...
    if (value?.type === "Manga") {
      return fromPostcardManga(value.value);
    }
    if (value?.type === "Url") {
      return {
        key: value.value,
        title: link.title,
        cover: link.image_url ?? void 0,
        url: value.value
      };
    }
    if (link.title) {
  ...
  function fromPostcardHomeComponent(component) {
    ...
    switch (value.type) {
      case "Scroller": {
        const entries2 = value.value.entries.map(fromPostcardLink).filter((entry) => entry != null);
        return {
          title,
          subtitle,
          kind: "scroller",
      ...
      case "ImageScroller": {
        const entries2 = value.value.links.map(fromPostcardLink).filter((entry) => entry != null);
        return {
          title,
          subtitle,
          kind: "scroller",
      ...
      case "MangaChapterList":
        return {
          ...
          subtitle,
          kind: "mangaList",
          entries: value.value.entries.map((entry) => fromPostcardManga(entry.manga)),
          listing: value.value.listing ? fromPostcardListing(value.value.listing) : void 0,
          pageSize: value.value.page_size ?? void 0
        };
      default:
  ...
  // parsers/aidoku/wasm-host/worker.ts
  var fetchBridge = createWorkerFetchBridge();
  ...
  self.addEventListener("message", async (event) => {
    ...
    const env = new WasmEnv(fetchBridge);
    try {
      ...
      self.postMessage({ type: "result", id: message.id, data });
    } catch (error) {
      self.postMessage({
        type: "error",
        id: message.id,
